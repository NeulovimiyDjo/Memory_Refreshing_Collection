...
#define MANAGED_ASSEMBLY "ManagedLib.dll"
...
int main(int argc, char* argv[])
{
    ...
    hr = createManagedDelegate(
        hostHandle,
        domainId,
        "ManagedLib, Version=1.0.0.0",
        "ManagedLib.ManagedWorker",
        "DoWork",
    ...
    std::string managedLibExePath(runtimePath);
    managedLibExePath.append(FS_SEPARATOR);
    managedLibExePath.append("ManagedLib.dll");

    int argc2 = 1;
    const char* argv2[] = {argv[0]};
    unsigned int exitCode;
    int hr2 = executeAssembly(
        hostHandle,
        domainId,
        argc2,
        (const char **)(&argv2),
        managedLibExePath.c_str(),
        (unsigned int*)&exitCode
    );
