...
namespace CommunicatorLib
{
    public class Communicator : ICommunicator, IV8EventHandler, IDisposable
    {
        ...
        public async Task Connect(CancellationToken cancellationToken)
        {
            ...
            var uri = new Uri(tabInfo.WebSocketDebuggerUrl ?? $"ws://{_hostname}:{_port}/{tabInfo.Id}");

            _socket = new ClientWebSocket{ Options = { KeepAliveInterval = TimeSpan.FromSeconds(0) } };
