    ...
    getters: {
        filteredRaces: (state, getters, rootState, rootGetters) => {
            return Object.values(state.database.races).filter((race) => {
                let alreadyChosen =
                    rootState['character'].character.race.id === race.id;
        ...
        filteredSubraces: (state, getters, rootState, rootGetters) => {
            ...
            return Object.values(subraces).filter((subrace) => {
                let alreadyChosen =
                    rootState['character'].character.subrace.id === subrace.id;
        ...
        filteredFeats: (state, getters, rootState, rootGetters) => {
            return state.database.feats.filter((feat) => {
                let alreadyChosen =
                    rootState['character'].character.feats.findIndex(
                        (f) => f.id === feat.id
                    ) !== -1;

                let options = rootState['character'].character.raceOptions;
                Object.keys(options).forEach((abilityName) => {
                    let alreadyChosenByRaceOption =
                        options[abilityName].findIndex(
                            (o) => o.name === feat.name
                        ) !== -1;
                ...
                let satisfiesCharacterConfig = rootGetters[
                    'character/satisfiesCharacterConfig'
                ](feat, 'feat');

                return (
                    satisfiesCharacterConfig &&
                    (!alreadyChosen || canHaveMultiple(feat.name))
                );
            });
        },
        ...
        filteredSubraceAbilities: (state, getters, rootState, rootGetters) => {
            let subraceAbilities =
                rootState['character'].character.subrace.abilities;
        ...
        filteredClassAbilities: (state, getters, rootState, rootGetters) => (
            index
        ) => {
            let classAbilities =
                rootState['character'].character.classes[index].class.abilities;
            if (!classAbilities) return [];

            return classAbilities.filter((ability) => {
                let satisfiesCharacterConfig = rootGetters[
                    'character/satisfiesCharacterConfig'
                ](ability, 'classAbility', index);

                return satisfiesCharacterConfig;
            });
        },

        filteredSubclassAbilities: (state, getters, rootState, rootGetters) => (
            index
        ) => {
            let subclassAbilities =
                rootState['character'].character.classes[index].subclass
                    .abilities;
        ...
        filteredOptions: (state, getters, rootState, rootGetters) => (
            ability
        ) => {
            if (ability.name === 'Divine Magic') {
                let index = rootState['character'].character.classes.findIndex(
                    (c) => c.subclass.name === 'Divine Soul'
                );
                return ability.options.filter((option) => {
                    let alreadyChosen = false;
                    for (
                        let i = 0;
                        i < rootState['character'].character.classes.length;
                        i++
                    ) {
                        let alreadyChosenByClass =
                            rootState['character'].character.classes[
                                i
                            ].spells.findIndex((s) => s.id === option.id) !==
                            -1;
                ...
                return ability.options.filter((option) => {
                    ...
                    for (
                        ...
                        let options =
                            rootState['character'].character.classes[i].options;
                        Object.keys(options).forEach((abilityName) => {
                            let alreadyChosenByClass =
                                options[abilityName].findIndex(
        ...
        filteredClasses: (state, getters, rootState, rootGetters) => {
            return Object.values(state.database.classes).filter((cls) => {
                let alreadyChosen =
                    rootState['character'].character.classes.findIndex(
                        (c) => c.class.id === cls.id
                    ) !== -1;
                let satisfiesCharacterConfig = rootGetters[
                    'character/satisfiesCharacterConfig'
        ...
        filteredSubclasses: (state, getters, rootState, rootGetters) => (
            index
        ) => {
            let subclasses =
                rootState['character'].character.classes[index].class
                    .subclasses;
        ...
        filteredCantrips: (state, getters, rootState, rootGetters) => (
            index
        ) => {
            return state.database.cantrips.filter((cantrip) => {
                ...
                for (
                    ...
                    let alreadyChosenByClass =
                        rootState['character'].character.classes[
                            i
                        ].cantrips.findIndex((c) => c.id === cantrip.id) !== -1;
            ...
            return state.database.spells.filter((spell) => {
                ...
                for (
                    ...
                    let chosenInSpells =
                        rootState['character'].character.classes[
                            i
                        ].spells.findIndex((s) => s.id === spell.id) !== -1;

                    let options =
                        rootState['character'].character.classes[i].options;
                    let chosenInOptions =
                        Object.keys(options).findIndex(
                            (oName) =>
                                options[oName].findIndex(
                                    (o) => o.id === spell.id
                                ) !== -1
                        ) !== -1;
        ...
        load({ commit }) {
            let loadedPromise = new Promise((resolve) => {
                api.getDndDatabase().then((db) => {
                    ...
                    database.races['Human'].subraces[
                        'Standard Human'
                    ].abilities.find(
                        (a) => a.name === 'Ability Score Increase'
                    ).bonusStats = bonusStatsAllOne;

                    database.races['Half-Elf'].abilities.find(
                        (a) => a.name === 'Ability Score Increase'
                    ).options = scoreIncreaseVariant;
                    ...
                    database.races['Human'].subraces[
                        'Variant Human'
                    ].abilities.find((a) => a.name === 'Feat').options =
                        db.feats;

                    let dm = database.classes['Sorcerer'].subclasses[
                        'Divine Soul'
                    ].abilities.find((a) => a.name === 'Divine Magic');
                    dm.options = database.spells.filter(
                        (s) => s.classes.findIndex((c) => c === 'Cleric') !== -1
                    );

                    let afs = database.classes['Fighter'].subclasses[
                        'Champion'
                    ].abilities.find(
                        (a) => a.name === 'Additional Fighting Style'
                    );
                    afs.options = database.classes['Fighter'].abilities.find(
                        (a) => a.name === 'Fighting Style'
                    ).options;
