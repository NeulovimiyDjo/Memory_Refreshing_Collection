    ...
    <div class="stat-chooser">
        <div v-if="statsPlus1x2Items.includes(item.name)">
            <div style="display: flex; direction: row">
                ...
                >
                    <option
                        v-for="stat in statTypesMinusSelected2"
                        :key="stat"
                        :value="stat"
                    >
                        {{ stat }}
            ...
            <div style="display: flex; direction: row">
                ...
                <select
                    @click.stop
                    :value="selected2"
                    @change="selected2Changed($event)"
                >
                    <option
                        v-for="stat in statTypesMinusSelected1"
                        :key="stat"
                        :value="stat"
                    >
                        {{ stat }}
        ...
        <div v-if="statsPlus2x1Items.includes(item.name)">
            <div style="display: flex; direction: row">
                ...
                >
                    <option
                        v-for="stat in statTypesFiltered"
                        :key="stat"
                        :value="stat"
                    >
                        {{ stat }}
...
export default {
    ...
    methods: {
        ...
        changeItem() {
            if (
                statsPlus1x2Items
                    .concat(statsPlus2x1Items, statsPlus1x1Items)
