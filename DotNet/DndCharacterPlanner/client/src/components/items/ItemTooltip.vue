...
export default {
    ...
    mounted() {
        if (this.text.length > 1050) {
            ...
            this.$el.style.width =
                Math.min(this.text.length / 30, max) + 'vmin';
            this.$el.style.left =
                -1.5 - Math.min(this.text.length / 30, max) + 'vmin';
        }
        ...
        if (this.posX > 0) {
            ...
            let left =
                ...
                this.$parent.$el.getBoundingClientRect().left;
    ...
    computed: {
        text() {
            ...
            if (this.moduleType === 'cantrips') {
                ...
                this.moduleType === 'spells' ||
                (this.moduleType === 'options' && this.item.level)
            ) {
            ...
            if (this.item.source)
                res = res + '</br>Source: ' + this.item.source + '<br/>';
            ...
            if (this.item.components)
                res = res + '</br>Components: ' + this.item.components;
            if (this.item.duration)
                res = res + '</br>Duration: ' + this.item.duration + '</br>';
            ...
            if (this.item.description) {
                let fontSize = Math.max(
                    Math.min(3000 / this.item.description.length, 1.4),
                    1.2
                );
                res =
                    res +
                    '</br>Description:<pre style="font-size: ' +
                    fontSize +
                    'vmin;">' +
            ...
            if (this.item.bonusStats) {
                if (Object.keys(this.item.bonusStats).length > 0)
                    res = res + '</br>Stats:';
