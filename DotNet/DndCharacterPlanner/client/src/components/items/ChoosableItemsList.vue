<template>
    <div>
        ...
        <ul>
            <choosable-item
                v-for="(item, index) in choosableSource"
                :key="index"
                :slotId="index"
                :item="item"
                :currentlyClickedSlotId="currentlyClickedSlotId"
                @slot-clicked="slotClickedHandler"
                :moduleType="moduleType"
                class="choosable-item"
                :popups="popups"
            />
        </ul>
        ...
        <div :style="{ top: avaliableItemsPosY + 'px', position: 'absolute' }">
            <available-items-list
                v-show="currentlyClickedSlotId !== -1"
                ref="available-items"
                :availableSource="availableSource"
                :slotId="currentlyClickedSlotId"
                :moduleType="moduleType"
    ...
    methods: {
        slotClickedHandler({ slotId, posY }) {
            ...
            this.$nextTick(function () {
                // wait for display change back to visible
                this.$refs['available-items'].$refs[
                    'scrollable-ul'
                ].scrollTop = 0;
            });
