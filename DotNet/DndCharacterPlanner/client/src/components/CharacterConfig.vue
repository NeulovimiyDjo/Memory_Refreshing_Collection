    ...
    <div>
        ...
        <div v-show="filteredSubraces.length > 0" class="container-single">
            ...
            <choosable-items-list
                :moduleType="'subrace'"
                :moduleId="'subrace'"
                :choosableSource="[character.subrace]"
                :availableSource="filteredSubraces"
        ...
        <div
            style="background-color: olive; margin: 1vmin 0 1vmin 0"
        ...
        >
            <div class="container-single">
                ...
                <choosable-items-list
                    :moduleType="'class'"
                    :moduleId="'class' + classIndex"
                    :choosableSource="[character.classes[classIndex].class]"
                    :availableSource="filteredClasses"
                    @item-chosen="
                        setClass({ classIndex: classIndex, cls: $event.item })
                    "
                    :lastModuleToClickItem="lastModuleToClickItem"
                    @slot-clicked="$emit('slot-clicked', $event)"
                    class="choosable-items-list list-single"
            ...
            <div
                v-show="filteredSubclasses(classIndex).length > 0"
                class="container-single"
            >
                <p>Subclass</p>
                <choosable-items-list
                    :moduleType="'subclass'"
                    :moduleId="'subclass' + classIndex"
            ...
            <div class="container-single" style="margin: 1vmin 0 1vmin 0">
                <p>Class Level</p>
                <select
                    :value="character.classes[classIndex].level"
                    @change="levelChangedHandler(classIndex, $event)"
                    style="margin: 0.5vmin"
                >
                    <option
                        v-for="lvl in possibleLevels(classIndex)"
                        :key="lvl"
                        :value="lvl"
                    >
                        {{ lvl }}
        ...
        possibleLevels(index) {
            return this.levelsList.filter(
                (lvl) =>
                    this.totalLevel +
                        lvl -
                        this.character.classes[index].level <=
                    20
            );
        },
    },
