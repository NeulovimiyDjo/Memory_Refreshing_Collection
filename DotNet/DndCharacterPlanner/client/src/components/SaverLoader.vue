...
function addBonusStatsFor(items, bonusStats, prefix) {
    for (let i = 0; i < items.length; i++) {
        if (items[i].bonusStats) {
            bonusStats[prefix + ':' + items[i].name + ':' + i] =
                items[i].bonusStats;
        }
    }
}
...
export default {
    ...
    computed: {
        ...mapState('character', ['changed', 'character']),

        ...mapState('database', ['database']),
    ...
    methods: {
        ...
        saveHandler() {
            ...
            Object.keys(this.character.raceOptions).forEach((abilityName) => {
                raceOptions[abilityName] = this.character.raceOptions[
                    abilityName
                ].map((o) => (o.id === -1 ? '' : o.name));
            });
            ...
            let char = {
                race:
                    this.character.race.id === -1
                        ? ''
                        : this.character.race.name,
                subrace:
                    this.character.subrace.id === -1
                        ? ''
                        : this.character.subrace.name,
                stats: this.character.stats,
                bonusStats: getBonusStats(this.character),
                feats: this.character.feats.map((feat) =>
                    feat.id === -1 ? '' : feat.name
                ),
                ...
                classes: this.character.classes.map((c) => {
                    ...
                    Object.keys(c.options).forEach((abilityName) => {
                        options[abilityName] = c.options[abilityName].map((o) =>
                            o.id === -1 ? '' : o.name
                        );
                    });
                    ...
                    return {
                        ...
                        level: c.level,
                        cantrips: c.cantrips.map((cantrip) =>
                            cantrip.id === -1 ? '' : cantrip.name
                        ),
                        spells: c.spells.map((spell) =>
                            spell.id === -1 ? '' : spell.name
                        ),
                        options: options,
                    };
                }),
            ...
            if (this.changed) {
                api.saveCharacter(char).then((id) => {
                    this.characterId = id;
                    this.$router.push({
                        name: 'planner',
                        params: { id: String(id) },
        ...
        loadCharacter(charId) {
            api.getCharacter(charId).then((char) => {
                let character = {
                    race: nameToItem(
                        char.race,
                        Object.values(this.database.races)
                    ),
                    stats: char.stats,
                    feats: char.feats.map((name) =>
                        nameToItem(name, this.database.feats, 'deepCopy')
                    ),
                    ...
                    classes: char.classes.map((c) => {
                        let cls = {
                            class: nameToItem(
                                c.class,
                                Object.values(this.database.classes)
                            ),
                            level: c.level,
                            cantrips: c.cantrips.map((name) =>
                                nameToItem(name, this.database.cantrips)
                            ),
                            spells: c.spells.map((name) =>
                                nameToItem(name, this.database.spells)
                            ),
                        };
                        ...
                        cls.subclass = cls.class.subclasses
                            ? nameToItem(
                                  c.subclass,
                                  Object.values(cls.class.subclasses)
                              )
                            : { id: -1 };

                        let options = {};
                        Object.keys(c.options).forEach((abilityName) => {
                            let ability = nameToItem(
                                abilityName,
                                cls.class.abilities.concat(
                                    cls.subclass.abilities
                                )
                            );
                            options[abilityName] = c.options[abilityName].map(
                                (optionName) => {
                                    return nameToItem(
                                        optionName,
                                        ability.options,
                ...
                character.subrace = character.race.subraces
                    ? nameToItem(
                          char.subrace,
                          Object.values(character.race.subraces)
                      )
                    : { id: -1 };
                ...
                Object.keys(char.raceOptions).forEach((abilityName) => {
                    let ability = nameToItem(
                        abilityName,
                        character.race.abilities.concat(
                            character.subrace.abilities
                        )
                    );
                    raceOptions[abilityName] = char.raceOptions[
                        abilityName
                    ].map((optionName) => {
                        return nameToItem(
                            optionName,
