function makeNewList(amount, state, getters, type, index) {
    ...
    for (let i = 0; i < newList.length; i++) {
        if (
            i < oldList.length &&
            getters.satisfiesCharacterConfig(oldList[i], itemType, index)
        ) {
            newList[i] = oldList[i];
        } else {
            newList[i] = { id: -1 };
...
function setNewOptions(state, commit) {
    {
        ...
        if (
            addOptions(
                ...
                oldOptions,
                1,
                state.character.race.abilities
            )
        )
            changed = true;
        if (
            addOptions(
                ...
                oldOptions,
                1,
                state.character.subrace.abilities
            )
        )
            changed = true;
        ...
        if (changed) commit('setRaceOptionsObject', newOptions);
    }
    ...
    for (let i = 0; i < state.character.classes.length; i++) {
        ...
        if (
            addOptions(
                ...
                oldOptions,
                classLvl,
                state.character.classes[i].class.abilities
            )
        )
            changed = true;
        if (
            addOptions(
                ...
                oldOptions,
                classLvl,
                state.character.classes[i].subclass.abilities
            )
        )
            changed = true;
...
function optionSatisfiesCharacterConfig(option, ability, classLvl) {
    let isForCurrentAbility =
        ability.options.findIndex((o) => o.id === option.id) !== -1;
...
function addOptions(newOptions, oldOptions, classLvl, abilities) {
    ...
    abilities.forEach((a) => {
        ...
        for (let i = 0; i < newList.length; i++) {
            if (
                i < oldList.length &&
                optionSatisfiesCharacterConfig(oldList[i], a, classLvl)
            ) {
                newList[i] = oldList[i];
            } else {
                changed = true;
...
function getListAmount(state, getters, i, type) {
    ...
    switch (type) {
        case 'feats':
            ...
            increases = feats.filter(
                (lvl) => lvl <= state.character.classes[i].level
            );
            break;
        case 'cantrips':
            if (
                state.character.classes[i].subclass.cantrips &&
                state.character.classes[i].subclass.cantrips.length > 0
            ) {
                let cantrips = state.character.classes[i].subclass.cantrips;
                increases = cantrips.filter(
                    (lvl) => lvl <= state.character.classes[i].level
                );
            } else {
        ...
        case 'spells':
            switch (state.character.classes[i].class.name) {
                ...
                case 'Wizard':
                    return Math.max(
                        getters.realStatModifiers['int'] +
                            state.character.classes[i].level,
                        1
                    );
                case 'Cleric':
                case 'Druid':
                    return Math.max(
                        getters.realStatModifiers['wis'] +
                            state.character.classes[i].level,
                        1
                    );
                case 'Paladin':
                    return Math.max(
                        getters.realStatModifiers['cha'] +
                            state.character.classes[i].level,
                        1
                    );
            }
            if (
                state.character.classes[i].subclass.spells &&
                state.character.classes[i].subclass.spells.length > 0
            ) {
                let spells = state.character.classes[i].subclass.spells;
                increases = spells.filter(
                    (lvl) => lvl <= state.character.classes[i].level
                );
            } else {
...
function bonusValues(state, rootGetters) {
    ...
    {
        modifyBonusValuesFrom(
            rootGetters['database/filteredRaceAbilities'],
            bonusValues
        );
        modifyBonusValuesFrom(
            rootGetters['database/filteredSubraceAbilities'],
            bonusValues
        );
    ...
    for (let i = 0; i < state.character.classes.length; i++) {
        modifyBonusValuesFrom(
            rootGetters['database/filteredClassAbilities'](i),
            bonusValues
        );
        modifyBonusValuesFrom(
            rootGetters['database/filteredSubclassAbilities'](i),
            bonusValues
        );
...
export default {
    ...
    getters: {
        realStatScores(state, getters, rootState, rootGetters) {
            ...
            Object.keys(state.character.stats).forEach((statName) => {
                res[statName] = Math.min(
                    state.character.stats[statName] +
                        bonusValues(state, rootGetters)[statName],
                    20
                );
            });
        ...
        realStatModifiers(state, getters) {
            let res = {};
            Object.keys(state.character.stats).forEach((statName) => {
                res[statName] = Math.floor(
                    (getters.realStatScores[statName] - 10) / 2
                );
            });
        ...
        canAddClass(state, getters, rootState) {
            return (
                getters.totalLevel < 20 &&
                state.character.classes.length <
                    Object.keys(rootState['database'].database.classes).length
            );
        },
        ...
        satisfiesCharacterConfig: (state) => (item, type, index) => {
            switch (type) {
                ...
                case 'classAbility': {
                    let enoughLevel =
                        item.level <= state.character.classes[index].level;
                ...
                case 'subclass': {
                    ...
                    if (state.character.classes[index].class.subclasses) {
                        isForCurrentClass =
                            Object.values(
                                state.character.classes[index].class.subclasses
                            ).findIndex((sub) => sub.id === item.id) !== -1;
                    }
                ...
                case 'spell': {
                    ...
                    let isDivineSoul =
                        ...
                        'Divine Soul';
                    let isForCurrentClass =
                        Object.values(item.classes).findIndex(
                            (c) =>
                                c === className ||
                                (isDivineSoul && c === 'Cleric')
                        ) !== -1;
                    ...
                    let enoughLevel =
                        item.level * 2 - 1 <=
                        state.character.classes[index].level;
    ...
    mutations: {
        ...
        setCantrip(state, { classIndex, pos, cantrip }) {
            state.character.classes[classIndex].cantrips.splice(
                pos,
                1,
                cantrip
            );
        ...
        setClassOption(state, { classIndex, pos, abilityName, option }) {
            state.character.classes[classIndex].options[abilityName].splice(
                pos,
                1,
                option
            );
        ...
        setCantripsList(state, { classIndex, cantrips }) {
            if (
                areDifferentLists(
                    state.character.classes[classIndex].cantrips,
                    cantrips
                )
            ) {
        ...
        setSpellsList(state, { classIndex, spells }) {
            if (
                areDifferentLists(
                    state.character.classes[classIndex].spells,
                    spells
                )
            ) {
        ...
        addClass(state) {
            state.character.classes.push(
                JSON.parse(JSON.stringify(emptyClass))
            );
            state.changed = true;
        },
    ...
    actions: {
        ...
        setAmounts({ state, getters, commit }) {
            ...
            for (let i = 0; i < state.character.classes.length; i++) {
                ...
                commit('setCantripsList', {
                    classIndex: i,
                    cantrips: makeNewList(
                        cantripsAmount,
                        state,
                        getters,
                        'cantrips',
            ...
            commit(
                'setFeatsList',
                makeNewList(totalFeatsAmount, state, getters, 'feats')
            );
        ...
        checkSubrace({ state, getters, commit }) {
            if (
                !getters.satisfiesCharacterConfig(
        ...
        checkSubclass({ state, getters, commit }) {
            for (let i = 0; i < state.character.classes.length; i++) {
                if (
                    !getters.satisfiesCharacterConfig(
                        state.character.classes[i].subclass,
                        'subclass',
                        i
                    )
                ) {
